...
app.post('/categories', async (req, res) => {
  const { categoryName, availableStates } = req.body;
  const timestamp = new Date(Date.now() + 8 * 60 * 60 * 1000);
  ...
  try {
    const result = await pool.query(
      `INSERT INTO categories (categoryname, availablestates, timestamp) 
       VALUES ($1, $2, $3) 
       RETURNING *`,
      [categoryName, availableStates || 'All States', timestamp]
    );
...
app.put('/categories/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE categories 
       SET categoryname = $1, availablestates = $2 
       WHERE categoryid = $3 
       RETURNING *`,
      [categoryName, availableStates || 'All States', id]
    );
...
app.delete('/categories/:id', async (req, res) => {
  ...
  try {
    // Check if there are any properties associated with this category
    const propertyCheck = await pool.query(
      'SELECT COUNT(*) FROM properties WHERE categoryid = $1',
      [id]
    );
    ...
    if (parseInt(propertyCheck.rows[0].count) > 0) {
      return res.status(400).json({
        success: false,
        message: 'Cannot delete category because it has associated properties'
      });
    }
